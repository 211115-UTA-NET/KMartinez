...
(
	...
	--DATEmotified
	DataMotified DATATIMEOFFSET NOT NULL DEFAULT (SYSDATETIMEOFFSET())
	--BookBizId AS (SUBSTRING(Title,1,3) + SUBSTRING(Author,1,3) + '000') PERSISTED,   -- computed column
	--Constrain
	--CONSTRAINT CK_Page_Positive CHECK (Pages > 0)
);
...
--CONSTRAINT to change name, makes it easier to drop later since the name wont be auto genreated
--ALTER TABLE Books ADD CONSTRAINT FK_Author FOREIGN KEY (Author) REFERENCES Authors(Author);
--ALTER TABLE Books DROP CONSTRAINT FK_Author FOREIGN KEY (Author) REFERENCES Authors(Author);
